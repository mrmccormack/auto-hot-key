...
SetLineNumberWidth(n) {
    If (g_LineNumbers) {
        ...
        If (LineCountLen != Sci[n].MarginLen) {
            ...
            PixelWidth := Sci[n].TextWidth(STYLE_LINENUMBER, "" . String, 1) + 6
...
SetCodeFolding(n, XPM := True) {
    Static XPMLoaded := 0, PixmapCollapsed, PixmapExpanded

    If (!XPMLoaded) {
        FileRead PixmapCollapsed, %A_ScriptDir%\Icons\Collapsed.xpm
        FileRead PixmapExpanded, %A_ScriptDir%\Icons\Expanded.xpm
        XPMLoaded := 1
    }

    Sci[n].SetMarginTypeN(2, SC_MARGIN_SYMBOL)
    Sci[n].SetMarginWidthN(2, 16)
    Sci[n].SetMarginMaskN(2, SC_MASK_FOLDERS)
    Sci[n].SetMarginSensitiveN(2, True)

    Loop 7 {
        i := A_Index + 24
        Sci[n].MarkerSetFore(i, CvtClr(0xFFFFFF))
        Sci[n].MarkerSetBack(i, CvtClr(0x008EBC)) ; Border
    }

    ;Sci[n].MarkerDefine(SC_MARKNUM_FOLDER, SC_MARK_BOXPLUS)
    Sci[n].MarkerDefine(30, 25) ; SC_MARK_PIXMAP
    Sci[n].MarkerDefinePixmap(30, "" . PixmapCollapsed, 1)

    ;Sci[n].MarkerDefine(SC_MARKNUM_FOLDEROPEN, SC_MARK_BOXMINUS)
    Sci[n].MarkerDefine(31, 25) ; SC_MARK_PIXMAP
    Sci[n].MarkerDefinePixmap(31, "" . PixmapExpanded, 1)

    ;Sci[n].MarkerDefine(SC_MARKNUM_FOLDEREND, SC_MARK_BOXPLUSCONNECTED)
    Sci[n].MarkerDefine(25, 25) ; SC_MARK_PIXMAP
    Sci[n].MarkerDefinePixmap(25, "" . PixmapCollapsed, 1)

    ;Sci[n].MarkerDefine(SC_MARKNUM_FOLDEROPENMID, SC_MARK_BOXMINUSCONNECTED)
    Sci[n].MarkerDefine(26, 25) ; SC_MARK_PIXMAP
    Sci[n].MarkerDefinePixmap(26, "" . PixmapExpanded, 1)
    ...
    ; Background color
    Sci[n].2290(1, CvtClr(0xFAFAFA)) ; SCI_SETFOLDMARGINCOLOUR
    Sci[n].2291(1, CvtClr(0xFAFAFA)) ; SCI_SETFOLDMARGINHICOLOUR
}
...
CloseTab(TabIndex, Confirm := True, Multiple := False, Exiting := False) {
    ...
    If (TabCount > 1) {
        ...
        If (ToActivate) {
            SendMessage 0x1330, ToActivate - 1,,, ahk_id %hTab% ; TCM_SETCURFOCUS.
            Sleep 0
...
OnWM_NOTIFY(wParam, lParam, msg, hWnd, obj) {
    ...
    If (obj.SCNCode == SCN_UPDATEUI) {
        ...
        /*
        If (g_ErrorMarker && obj.updated == 1) {
            g_ErrorMarker := False
            Sci[n].MarkerDeleteAll(3)
        }
        */
        ...
        ; Autoclose brackets ([{""}])
        Char := obj.ch
        If Char in 40,91,123,34 ; Parenthesis, brackets, braces, quotes
        {
        ...
        */
        ...
        If (obj.Margin == 1) {
            ...
            If (Sci[n].MarkerGet(Line) & (1 << Marker)) { ; Bookmarker marker
...
; Called from SCN_SAVEPOINTREACHED, SCN_SAVEPOINTLEFT and NewGUI
SetDocumentStatus(n) {
    Gui Auto: Default
...
ToggleReadOnly() {
    ...
    Menu AutoEditMenu, ToggleCheck, Set as &Read-Only
...
; GoToLine
ShowGoToLineDialog() {
    Line := InputBoxEx("Line Number:", "", "Go to Line", "", "", "x94 w80 Number", hAutoWnd, 270) - 1
    If (!ErrorLevel) {
        ShowChildWindow(0)
...
ToggleBookmark() {
    ...
    If (Sci[n].MarkerGet(Line) & (1 << g_MarkerBookmark)) {
        Sci[n].MarkerDelete(Line, g_MarkerBookmark)
    } Else {
        Sci[n].MarkerAdd(Line, g_MarkerBookmark)
    }
}
...
ClearAllMarks() {
    n := TabEx.GetSel()
    Sci[n].MarkerDeleteAll(0) ; Bookmarked lines
    Sci[n].SetIndicatorCurrent(1)
...
GetMarks() {
    ...
    Prev := Sci[n].MarkerPrevious(LineStart - 1, 1 << g_MarkerBookmark)
    If (Prev != -1) {
        Prev := Sci[n].PositionFromLine(Prev)
        Marks.Push(Prev)
    }

    Next := Sci[n].MarkerNext(LineStart + 1, 1 << g_MarkerBookmark) ; markerMask
    If (Next != -1) {
        Next := Sci[n].PositionFromLine(Next)
        Marks.Push(Next)
    }
...
ToggleComment() {
    ...
    Loop Parse, SelText, `n, `r
    {
        If (RegExMatch(A_LoopField, "^\s*\;")) {
            Line := RegExReplace(A_LoopField, "\;", "", "", 1) ; Uncomment
        } Else If (A_LoopField == "") {
            Line := ""
        } Else {
            RegExMatch(A_LoopField, "^[^\S]+", Indentation)
            NewStr := StrReplace(A_LoopField, Indentation, "",, 1)
            Line := Indentation . ";" . NewStr ; Comment
        }
        Lines .= Line . CRLF
    }
