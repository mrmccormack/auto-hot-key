...
DebugRun() {
    If (g_DbgStatus == 0) {
        ...
        RunWait "%AhkPath%" /debug=127.0.0.1:%g_DbgPort% "%File%", %WorkingDir%, UseErrorLevel
...
ShowVarList:
    ...
    Gui Add, CheckBox, vChkVarRegEx x346 y380 w100 h23, Using &RegEx
    ...
    If (g_NT6orLater) {
        ...
        DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hLVVarList, "WStr", "Explorer", "Ptr", 0)
    }
...
ModifyVariable:
    ...
    If (VarName = "A_LastError") {
        ...
        MsgBox 0, A_LastError: %VarValue%, % GetErrorMessage(VarValue)
        Return
    }
    ...
    If (Builtin) {
        Gui Variables: +OwnDialogs
        MsgBox 0x10, Modify Variable, "%VarName%" is a READ-ONLY built-in variable.
        Return
    }
...
ShowStderr:
    ...
    LV_ModifyCol(2, 112)
    LV_ModifyCol(3, 396)
    DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hLVStderr, "WStr", "Explorer", "Ptr", 0)
...
ShowCallStack:
    ...
    Gui Font, s9, Segoe UI
    Gui Add, ListView, hWndhLVCallStack x7 y8 w569 h280 +LV0x10000, File|Line|Stack Entry
    LV_ModifyCol(1, 100)
    LV_ModifyCol(2, 45)
    LV_ModifyCol(3, 396)
    DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hLVCallStack, "WStr", "Explorer", "Ptr", 0)
...
ShowDebugButtons(Show := 1) {
    ...
    Try {
        If (Show) {
            Menu AutoRunMenu, Rename, Start Debugging`tF5, Continue`tF5
        } Else {
            Menu AutoRunMenu, Rename, Continue`tF5, Start Debugging`tF5
        }
    }
}
...
SetDebugPort() {
    Port := InputBoxEx("Debug Port"
        , "Specify to port to be used by the debugger: (default: 9001)"
        , "Debug Settings"
        , g_DbgPort
        , ""
        , ""
...
ShowAttachDialog:
    ...
    DllCall("UxTheme.dll\SetWindowTheme", "Ptr", hLVAttachList, "WStr", "Explorer", "Ptr", 0)
